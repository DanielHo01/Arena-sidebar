...
- **LRU 上限** — 只保留最新的 50 条会话快照，超出的按 `lastSavedAt` 自动淘汰（重访该会话会从 DOM 重新提取）
- **配额重试** — 写入命中 quota 时自动清理到最新 10 条并重试一次；文件夹索引、重命名、隐藏标记体积小且不可再生，永不淘汰
