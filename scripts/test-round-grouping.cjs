...
runTest(
	"60 all-assistant → 1 round (the bug scenario)",
	1,
	roundsAllAssistant.length,
	{
		sample: {
			title: roundsAllAssistant[0].title,
			msgCount: roundsAllAssistant[0].messageCount,
		},
	},
);
...
runTest(
	"60 all-user → 60 rounds (each user message = own round)",
	60,
	roundsAllUser.length,
);
