...
async function waitForCDP() {
  for (let i = 0; i < 20; i++) {
    await sleep(800)
    try { await getJson('/json/version'); return true } catch {}
  }
  return false
}
...
async function getShadowNodeId(send, sess) {
  ...
  const host = await send('DOM.querySelector', { nodeId: docNodeId, selector: '#__edge_ai_sidebar_host' }, sess)
  if (!host || !host.nodeId || host.nodeId === 0) {
    console.log('getShadowNodeId: host not found via querySelector')
    return null
  }
  const described = await send('DOM.describeNode', { nodeId: host.nodeId, depth: 1, pierce: true }, sess)
  if (!described || !described.node || !described.node.shadowRoots || described.node.shadowRoots.length === 0) {
    console.log('getShadowNodeId: host found but no shadowRoots in describeNode')
    return null
  }
...
async function shadowBox(send, sess, shadowNodeId, selector) {
  const qr = await send('DOM.querySelector', { nodeId: shadowNodeId, selector }, sess)
...
async function shadowCount(send, sess, shadowNodeId, selector) {
  const qr = await send('DOM.querySelectorAll', { nodeId: shadowNodeId, selector }, sess)
...
async function run() {
  ...
  if (!(await waitForCDP())) {
    ...
    edgeProc.kill('SIGKILL'); server.close(); process.exit(1)
  }
  ...
  const tab = await send('Target.createTarget', { url: 'http://127.0.0.1:8000/arena-mock.html' })
  const sess = (await send('Target.attachToTarget', { targetId: tab.targetId, flatten: true })).sessionId
  ...
  const r1 = await send('Runtime.evaluate', {
    expression: 'JSON.stringify({tagged: document.querySelectorAll("[data-ai-sidebar-id]").length, hostExists: !!document.getElementById("__edge_ai_sidebar_host")})',
    returnByValue: true
  }, sess)
  ...
  const fabText = fabBox ? await shadowText(send, sess, shadowNodeId, '.fab', 'aria-label') : null
  const s2 = { fabFound: !!fabBox, fabText: fabText || '', fabRect: fabBox ? { x: fabBox.x, y: fabBox.y, w: fabBox.w, h: fabBox.h } : null }
  ...
  console.log('--- 3. Click FAB at (' + Math.round(fabX) + ', ' + Math.round(fabY) + ') ---')
  await send('Input.dispatchMouseEvent', { type: 'mousePressed', x: fabX, y: fabY, button: 'left', clickCount: 1 }, sess)
  await send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: fabX, y: fabY, button: 'left', clickCount: 1 }, sess)
  await sleep(600)

  // 4. Check panel + rounds via CDP DOM
  const panelBox = await shadowBox(send, sess, shadowNodeId, '.panel')
  const panelTitle = panelBox ? await shadowText(send, sess, shadowNodeId, '.title') : null
  const roundsCount = panelBox ? await shadowCount(send, sess, shadowNodeId, '.item') : 0
  const firstItemText = roundsCount > 0 ? await shadowText(send, sess, shadowNodeId, '.item') : null
  const s3 = { panelFound: !!panelBox, panelTitle, roundsCount, firstItemText: firstItemText?.trim().slice(0, 80) }
  ...
  const summaryBtnBox = await shadowBox(send, sess, shadowNodeId, '.summary-btn')
  console.log('--- 4.5. Panel elements ---')
  console.log(JSON.stringify({ summaryBtnExists: !!summaryBtnBox, searchInputExists: !!await shadowBox(send, sess, shadowNodeId, '.search-input') }, null, 2))

  // Check content script's injected hook is active in main world (CSP-safe external script)
  const hookCheck = await send('Runtime.evaluate', {
    expression: 'JSON.stringify({ hooked: window.fetch.toString().includes("__aiSideApiCaptured"), flagged: !!window.__aiSideApiCaptured })',
    returnByValue: true
  }, sess)
  ...
  if (!hc.hooked) {
    console.log('ERROR: inject-hook.js not installed in main world — reload extension')
    bws.close(); edgeProc.kill('SIGKILL'); server.close(); process.exit(1)
  }
  ...
  const hookFired = (await send('Runtime.evaluate', {
    expression: '!!window.__aiSideApiCaptured', returnByValue: true
  }, sess)).result.value

  // Debug: check dataset state after fetch
  const dsCheck = await send('Runtime.evaluate', {
    expression: 'document.documentElement.dataset.aiSideApi ? "SET: " + document.documentElement.dataset.aiSideApi.slice(0, 120) : "NOT SET"',
    returnByValue: true
  }, sess)
  ...
  await send('Runtime.evaluate', { expression: '(e => { const i = document.createElement("i"); e.appendChild(i); e.removeChild(i) })(document.body)', returnByValue: true }, sess)
  await sleep(800)

  // Re-acquire shadow root and check button disabled property
  const sRoot2 = await getShadowNodeId(send, sess)
  const btn2 = await shadowBox(send, sess, sRoot2 || shadowNodeId, '.summary-btn')
  let btnDisabled = true
  if (btn2?.nodeId) {
    const attrs = await send('DOM.getAttributes', { nodeId: btn2.nodeId }, sess)
  ...
  console.log(JSON.stringify({ hookFired, btnDisabled, apiCaptureOk }, null, 2))
  if (!apiCaptureOk) {
    console.log('ERROR: API discovery pipeline broken (hookFired=' + hookFired + ' btnDisabled=' + btnDisabled + ')')
    bws.close(); edgeProc.kill('SIGKILL'); server.close(); process.exit(1)
  }
  ...
  console.log('--- 5. Click 1st round at (' + Math.round(itemX) + ', ' + Math.round(itemY) + ') ---')

  const before = (await send('Runtime.evaluate', { expression: 'window.scrollY', returnByValue: true }, sess)).result.value
  await send('Input.dispatchMouseEvent', { type: 'mousePressed', x: itemX, y: itemY, button: 'left', clickCount: 1 }, sess)
  await send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: itemX, y: itemY, button: 'left', clickCount: 1 }, sess)
  await sleep(1200)
  const after = (await send('Runtime.evaluate', { expression: 'window.scrollY', returnByValue: true }, sess)).result.value
  const flash = (await send('Runtime.evaluate', { expression: 'document.querySelectorAll(".ai-sidebar-flash").length', returnByValue: true }, sess)).result.value
  console.log('Scroll: before=' + before + ' after=' + after + ' | changed: ' + (before !== after))
  console.log('Flash class applied: ' + flash + ' elements')

  // 6. Take screenshot
  const ss = await send('Page.captureScreenshot', { format: 'png' }, sess)
  if (ss && ss.data) {
    fs.writeFileSync('D:/edge-ai-sidebar/.e2e-fab-screenshot.png', Buffer.from(ss.data, 'base64'))
  ...
  const evidence = {
    ...
    edge: ver.Browser,
    extraction: { tagged: s1.tagged, expected: 8, ok: s1.tagged === 8, hostExists: s1.hostExists },
    fab: { found: s2.fabFound, rect: s2.fabRect },
    panel: { found: s3.panelFound, title: s3.panelTitle, rounds: s3.roundsCount, expectedRounds: 4, firstItemPreview: s3.firstItemText },
    click: { scrollBefore: before, scrollAfter: after, scrollChanged: before !== after, flashCount: flash },
    stateAfter: s5
  }
  fs.writeFileSync('D:/edge-ai-sidebar/.e2e-fab-evidence.json', JSON.stringify(evidence, null, 2))
  console.log('\n=== EVIDENCE ===')
