...
(async function () {
	...
	const grabStore = (s) => {
		...
		for (let i = 0; i < s.length; i++) {
			...
			o[k] = {
				size: v.length,
				preview: SENSITIVE.test(k)
					? "[REDACTED, size=" + v.length + "]"
					: trunc(v, 400),
			};
		}
		return o;
	...
	const grabEl = (sel) => {
		...
		return {
			...
			text: trunc(el.innerText, 1500),
			html: trunc(el.outerHTML, 4000),
		};
	};
	...
	out.nav = Array.from(document.querySelectorAll('button, a, [role="button"]'))
		.slice(0, 100)
		.map((el) => ({
			tag: el.tagName,
			text: trunc((el.innerText || "").trim(), 60),
			href: el.href || null,
			aria: el.getAttribute("aria-label"),
		}))
		.filter((b) => b.text || b.aria || b.href);

	out.inputs = Array.from(
	...
	if (!saved)
		try {
			...
			a.download =
				"arena-dump" + location.pathname.replace(/[^a-z0-9]/gi, "_") + ".json";
