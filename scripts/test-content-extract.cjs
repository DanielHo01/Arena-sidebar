...
function detectRole(el) {
	...
	if (cls.includes("assistant") || cls.includes("ai") || cls.includes("bot"))
		return "assistant";
	if (cls.includes("system")) return "system";

	const roleChild = el.querySelector(
		'.role-label, [class*="role-label"], [class*="author"], [data-role], [class*="role"]',
	);
	if (roleChild) {
...
function extractText(el) {
	...
	clone
		.querySelectorAll('button, [role="button"], nav, [aria-hidden="true"]')
		.forEach((n) => n.remove());
...
function extractMessages(doc) {
	...
	for (const el of all) {
		...
		const id =
			el.getAttribute("data-ai-sidebar-id") ||
			"msg-" + idx + "-" + Math.random().toString(36).slice(2, 8);
...
const CHATGPT =
	'<!doctype html><html><body><div id="chat-area">' +
...
function runTest(html, label, opts) {
	...
	if (opts.expectCount)
		checks.expectedCount = messages.length === opts.expectCount;
	if (opts.expectRole)
		checks.roleCorrect = messages.every(
			(m, i) => m.role === opts.expectRole[i],
		);
	if (opts.expectNoButtons)
		checks.noButtons = !messages.some(
			(m) => m.content.includes("like") || m.content.includes("Home"),
		);
	if (opts.expectNoAriaHidden)
		checks.noAriaHidden = !messages.some((m) =>
			m.content.includes("hidden UI"),
		);
...
const r1 = runTest(CHATGPT, "ChatGPT-style", {
	expectCount: 3,
	expectRole: ["user", "assistant", "user"],
	expectNoButtons: true,
});
const r2 = runTest(ARENA_CLASS, "Arena-class", {
	expectCount: 3,
	expectRole: ["user", "assistant", "user"],
});
const r3 = runTest(ARENA_ROLE, "Arena-data-role", {
	expectCount: 4,
	expectRole: ["user", "assistant", "user", "assistant"],
});
const r4 = runTest(EDGE_CASE, "Edge-cases", {
	expectCount: 2,
	expectRole: ["user", "assistant"],
	expectNoButtons: true,
	expectNoAriaHidden: true,
});
