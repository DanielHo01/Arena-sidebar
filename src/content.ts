...
let apiConfig: { url: string; headers: Record<string, string>; bodySample: any } | null = null
let roundSummaries = new Map<string, { title: string; summary: string }>()
const hiddenRoundIds = new Set<string>()
let isSummarizing = false

// Main world injection happens via manifest: inject-hook.js at document_start, world: MAIN
// Content script (isolated world) reads from shared DOM dataset written by inject-hook.js

// Read API config from shared DOM (set by inject-hook.js in main world)
let lastWsTs = 0
let wsEvents: any[] = []
...
function checkApiCapture() {
  ...
  try {
    ...
    if (d?.url && d?.bodySample?.messages?.length) {
      apiConfig = { url: d.url, headers: d.headers || {}, bodySample: d.bodySample }
      const btn = shadowRoot?.querySelector('.summary-btn') as HTMLButtonElement | null
...
function checkWsCapture() {
  ...
  try {
    ...
    if (ev.ts && ev.ts !== lastWsTs) {
      ...
      console.log('[AI Sidebar] WS event:', ev.event, ev.channel, (typeof ev.data === 'string' ? ev.data : JSON.stringify(ev.data)).substring(0, 200))
...
function debugWs() {
  const ready = document.documentElement.dataset.aiSideHookReady
  const created = document.documentElement.dataset.aiSideWsCreated
  console.log('[AI Sidebar] WS debug: hookReady=' + (ready || 'no') + ' lastCreated=' + (created || 'none'))
}
...
function harvestModelNames(): number {
  const w = window as unknown as { __NEXT_DATA__?: any; __initialModels?: any[]; __NEXT_DATA__raw?: any; __next_f?: any[]; __self?: any }
  const candidates: any[] = []
  const tryPaths = [
    () => w.__NEXT_DATA__?.props?.pageProps?.initialModels,
    () => w.__NEXT_DATA__?.props?.pageProps?.initialModelAId ? w.__NEXT_DATA__.props.pageProps : null,
    () => w.__initialModels,
  ]
  for (const fn of tryPaths) {
    try {
      ...
      else if (v && typeof v === 'object' && Array.isArray(v.initialModels)) candidates.push(v.initialModels)
    } catch {}
  }
  try {
    ...
    if (nextDataEl) {
      ...
      const m = txt.match(/"initialModels"\s*:\s*\[([\s\S]{20,50000}?)\](?=[\s,}])/)
      if (m) {
  ...
  try {
    const allTexts = Array.from(document.querySelectorAll('script')).map(s => s.textContent || '').join('\n')
    const ms = allTexts.matchAll(/"publicName"\s*:\s*"([^"]+)"[^}]{0,200}"id"\s*:\s*"([^"]+)"/g)
    for (const m of ms) {
  ...
  for (const arr of candidates) {
    ...
    for (const m of arr) {
      if (m && typeof m === 'object' && m.id && m.publicName) {
        if (!modelNameById.has(m.id)) { modelNameById.set(m.id, m.publicName); n++ }
      }
    }
  }
  return n
...
function checkChatCapture() {
  // ponytail: dataset poll — fast-path, simpler than MutationObserver for now
  ...
  if (reqRaw) {
    try {
      ...
      if (r.ts && r.ts !== lastRequestTs) {
        ...
        console.log('[AI Sidebar] Chat request:', r.sessionId, 'mode=' + (r.mode || '(empty)') + ' modality=' + r.modality, '| user:', (r.content || '').substring(0, 80))
      }
    } catch {}
  }
    ...
    try {
      ...
      if (resp.ts && resp.ts !== lastResponseTs) {
        ...
        for (const [sid, req] of pendingRequests) {
          ...
          const done = !!resp.aError || !!resp.bError ||
                       (isTwoModel ? (resp.aFinished && resp.bFinished) : resp.aFinished)
          chatRounds.set(sid, { sessionId: sid, request: req, response: resp, completed: done })
          const aName = lookupModelName(req.modelAId) || req.modelAId.slice(0, 8)
          const bName = req.modelBId ? (lookupModelName(req.modelBId) || req.modelBId.slice(0, 8)) : ''
          const tag = (done ? 'COMPLETE' : 'streaming') + ' [' + aName + (bName ? ' vs ' + bName : '') + ']'
          console.log('[AI Sidebar] Chat round ' + tag + ':', sid, '| a:', resp.aText.length, 'r:', resp.aReasoning.length, 'b:', resp.bText.length)
...
const USER_MESSAGE_SELECTOR = 'main [class*="bg-surface-raised"][class*="rounded-lg"]:not([class*="w-4"]):not([class*="inline-flex"])'
const ASSISTANT_MESSAGE_SELECTOR = 'main [class*="bg-surface-primary"][class*="flex-col"][class*="overflow-hidden"]'
...
function dedupeByRoot(elements: Element[]): Element[] {
  ...
  for (const el of elements) {
    ...
    for (const other of seen) {
      if (other.contains(el) || el.contains(other)) { dup = true; break }
    }
    if (!dup) { seen.add(el); out.push(el) }
  }
  return out
}

function collectAllElements(): Array<{ el: Element; role: 'user' | 'assistant' }> {
  const all: Array<{ el: Element; role: 'user' | 'assistant' }> = []
  const MIN_USER_LEN = 3
  const MIN_ASSISTANT_LEN = 50

  const isLikelyRealMessage = (el: Element, role: 'user' | 'assistant'): boolean => {
  ...
  return dedupedEls.map(el => ({ el, role: roleByEl.get(el) || 'assistant' }))
}
...
function groupIntoRounds(messages: ExtractedMessage[]) {
  ...
  let current: { id: string; title: string; messageCount: number } | null = null
  ...
  for (const msg of messages) {
    ...
    if (msg.role === 'user') {
      if (current !== null) {
        ...
        rounds.push({ id: pendingLeadAssistant.id, title: '(开场助手消息)', messageCount: 1 })
      }
      current = { id: msg.id, title: msg.content.slice(0, 80), messageCount: 0 }
...
function scrollToMessage(id: string) {
  const el = cachedElements.get(id) ?? document.querySelector('[data-ai-sidebar-id="' + CSS.escape(id) + '"]')
  if (!el) return
...
const ICON_MESSAGE_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>'

const ICON_X_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>'

let fabDragX = 0; let fabDragY = 0

function loadFabPosition() {
  if (typeof chrome !== 'undefined' && chrome.storage) {
    chrome.storage.local.get('fabPosition', (r: { fabPosition?: { x: number; y: number } }) => {
...
function buildSummaryPrompt(): string {
  ...
  currentMessages.forEach((m, idx) => { if (m.role === 'user') userIdxList.push(idx) })
  if (userIdxList.length === 0) return ''
  const capturedByUser = new Map<string, CapturedRound>()
  for (const r of chatRounds.values()) {
    if (r.request.content) capturedByUser.set(r.request.content.slice(0, 200), r)
  }
  const aResolved = Array.from(chatRounds.values()).map(r => lookupModelName(r.request.modelAId)).filter(Boolean)
  const bResolved = Array.from(chatRounds.values()).map(r => lookupModelName(r.request.modelBId)).filter(Boolean)
  const aLabel = aResolved[0] || '助手 A'
  const bLabel = bResolved[0] || '助手 B'
  let text = '请用中文为以下对话的每一轮生成一个 JSON 总结。每个 round 一个对象，包含 title (≤40 字要点)、summary (≤100 字简述)。\n输出格式：{"rounds":[{"id":"...","title":"...","summary":"..."}]}\n\n'
  let leadContext = ''
  ...
  userIdxList.forEach((userIdx, i) => {
    ...
    const assistants = currentMessages.slice(userIdx + 1, nextUserIdx).filter(m => m.role === 'assistant')
    ...
    if (assistants.length > 0) {
      ...
      if (cap.request.modelBId) text += `[${bLabel} 回答]: ${cap.response.bText.slice(0, 1500)}\n`
    }
  })
  return text
}
...
function exportConversation() {
  ...
  const buildJson = () => ({
    ...
    rounds: rounds.map((round, i) => {
      const userIdx = currentMessages.findIndex(m => m.id === round.id && m.role === 'user')
      const userContent = userIdx >= 0 ? currentMessages[userIdx].content : round.title
      const cap = capturedByUser.get(userContent.slice(0, 200))
      let assistants: ExtractedMessage[] = []
      if (userIdx >= 0) {
        const nextUserIdx = userIdxList[userIdxList.indexOf(userIdx) + 1] ?? currentMessages.length
        assistants = currentMessages.slice(userIdx + 1, nextUserIdx).filter(m => m.role === 'assistant')
      }
      return {
        round: i + 1,
        ...
        responses: assistants.length > 0 ? assistants.map(a => ({
          text: a.content,
        })) : (cap ? [
          { model: 'A', id: cap.request.modelAId, name: lookupModelName(cap.request.modelAId), text: cap.response.aText, reasoning: cap.response.aReasoning },
          ...(cap.request.modelBId ? [{ model: 'B', id: cap.request.modelBId, name: lookupModelName(cap.request.modelBId), text: cap.response.bText, reasoning: cap.response.bReasoning }] : []),
        ] : []),
      }
    }),
  })
  const json = JSON.stringify(buildJson(), null, 2)
  const md = buildJson().rounds.map((r) => {
...
function showExportModal(json: string, md: string, sid: string) {
  ...
  title.textContent = '📤 Export conversation — ' + (sid ? sid.slice(0, 8) + '…' : 'no session')
  const actions = document.createElement('div')
  actions.className = 'summary-actions'
  const copyJsonBtn = document.createElement('button')
  copyJsonBtn.textContent = '📋 Copy JSON'
  copyJsonBtn.onclick = () => navigator.clipboard.writeText(json).then(() => { copyJsonBtn.textContent = '✅ JSON copied'; setTimeout(() => { copyJsonBtn.textContent = '📋 Copy JSON' }, 1500) })
  const copyMdBtn = document.createElement('button')
  copyMdBtn.textContent = '📋 Copy Markdown'
  copyMdBtn.onclick = () => navigator.clipboard.writeText(md).then(() => { copyMdBtn.textContent = '✅ MD copied'; setTimeout(() => { copyMdBtn.textContent = '📋 Copy Markdown' }, 1500) })
  const copyLinkBtn = document.createElement('button')
  copyLinkBtn.textContent = '🔗 Copy link'
  copyLinkBtn.onclick = () => navigator.clipboard.writeText(location.href).then(() => { copyLinkBtn.textContent = '✅ Link copied'; setTimeout(() => { copyLinkBtn.textContent = '🔗 Copy link' }, 1500) })
  const dlJsonBtn = document.createElement('button')
  dlJsonBtn.textContent = '💾 Download JSON'
  dlJsonBtn.onclick = () => downloadBlob('arena-' + (sid || 'chat') + '.json', 'application/json;charset=utf-8', json)
  const dlMdBtn = document.createElement('button')
  dlMdBtn.textContent = '💾 Download .md'
  dlMdBtn.onclick = () => downloadBlob('arena-' + (sid || 'chat') + '.md', 'text/markdown;charset=utf-8', md)
...
function showSummaryModal(promptText: string) {
  ...
  title.textContent = '✨ Summary prompt — ' + roundCount + ' rounds · ' + promptText.length + ' chars'
  const actions = document.createElement('div')
  actions.className = 'summary-actions'
  const copyBtn = document.createElement('button')
  copyBtn.textContent = '📋 Copy to clipboard'
  copyBtn.onclick = () => {
    navigator.clipboard.writeText(promptText).then(() => { copyBtn.textContent = '✅ Copied'; setTimeout(() => { copyBtn.textContent = '📋 Copy to clipboard' }, 1500) })
  }
  const newTabBtn = document.createElement('button')
  newTabBtn.className = 'primary'
  newTabBtn.textContent = '🚀 Open new arena.ai chat'
  newTabBtn.onclick = () => {
    const url = 'https://arena.ai/?mode=direct&prompt=' + encodeURIComponent(promptText.slice(0, 4000))
    window.open(url, '_blank')
  }
  const pasteHereBtn = document.createElement('button')
  pasteHereBtn.textContent = '📥 Paste into current chat'
  pasteHereBtn.onclick = () => {
    const ta = document.querySelector('textarea[name="message"], textarea[placeholder*="followup" i], textarea[placeholder*="Ask" i]') as HTMLTextAreaElement | null
    if (ta) {
      ta.focus()
      ...
      setTimeout(() => { pasteHereBtn.textContent = '📥 Paste into current chat' }, 1500)
    } else {
...
async function summarizeRounds() {
  ...
  try {
    ...
    if (!prompt) {
      showSummaryModal('(no rounds detected yet — wait for arena.ai to load the conversation)')
      return
    }
...
function buildFab(): HTMLElement {
  ...
  btn.className = 'fab'
  if (fabPosition) {
    btn.style.right = 'auto'; btn.style.left = fabPosition.x + 'px'
    btn.style.top = fabPosition.y + 'px'; btn.style.transform = 'none'
  }
  btn.setAttribute('aria-label', 'Open message navigator (' + currentMessages.length + ' messages)')
  btn.title = currentMessages.length + ' messages'
  btn.innerHTML = ICON_MESSAGE_SVG
  btn.onclick = () => {
    isOpen = true
    refreshUI()
  }
  btn.addEventListener('mousedown', (e) => {
    ...
    e.preventDefault()
    isDragging = true
    fabDragX = e.clientX; fabDragY = e.clientY
    const move = (me: MouseEvent) => {
      const dx = me.clientX - fabDragX; const dy = me.clientY - fabDragY
      fabDragX = me.clientX; fabDragY = me.clientY
      const r = btn.getBoundingClientRect()
      btn.style.left = (r.left + dx) + 'px'; btn.style.top = (r.top + dy) + 'px'
...
function lookupRoundSummary(roundId: string): string {
  for (const r of chatRounds.values()) {
    if (r.request.content && (r.sessionId === roundId || (cachedElements.has(roundId) && cachedElements.get(roundId) === null))) {
...
function createRoundEl(round: { id: string; title: string; messageCount: number }, idx: number): HTMLElement {
  ...
  meta.textContent = round.messageCount + ' message' + (round.messageCount === 1 ? '' : 's')
  ...
  editBtn.onclick = (e) => {
    ...
    if (newTitle !== null && newTitle.trim()) {
      roundSummaries.set(round.id, { title: newTitle.trim().slice(0, 80), summary: '' })
      refreshUI()
    }
  }
  const copyBtn = document.createElement('button')
  ...
  copyBtn.onclick = (e) => {
    e.stopPropagation()
    const roundMsgs = currentMessages.filter(m => {
      const el = cachedElements.get(m.id)
      return el && (el === cachedElements.get(round.id) || el.contains(cachedElements.get(round.id)!) || cachedElements.get(round.id)!.contains(el))
    })
    const txt = roundMsgs.map(m => `[${m.role}] ${m.content}`).join('\n\n')
    if (txt) navigator.clipboard.writeText(txt).then(() => { copyBtn.textContent = '✅'; setTimeout(() => { copyBtn.textContent = '📋' }, 1200) })
  }
...
function updateRoundEl(el: HTMLElement, round: { id: string; title: string; messageCount: number }, idx: number) {
  const badge = el.querySelector('.badge')
  if (badge) badge.textContent = String(idx + 1)
  const meta = el.querySelector('.item-meta')
  if (meta) meta.textContent = round.messageCount + ' message' + (round.messageCount === 1 ? '' : 's')
...
function reconcileList(list: HTMLElement, rounds: Array<{ id: string; title: string; messageCount: number }>) {
  ...
  if (filtered.length === 0) {
    ...
    const msg = list.querySelector('.empty') || (() => { const e = document.createElement('div'); e.className = 'empty'; list.appendChild(e); return e })()
    msg.textContent = q ? 'No matches for "' + searchQuery + '"' : 'No messages detected'
    return
  }
  ...
  for (const [id, el] of byId) {
    if (!newIds.has(id)) { el.remove(); byId.delete(id) }
  }
...
function refreshCurrentHighlight() {
  ...
  for (const item of list.querySelectorAll('.item')) {
    (item as HTMLElement).classList.toggle('current', parseInt((item as HTMLElement).dataset.roundIdx || '-1') === currentRoundIdx)
  }
}
  ...
  list.addEventListener('scroll', () => {
    ...
    requestAnimationFrame(() => {
      ...
      items.forEach((item) => {
        ...
        const overlap = Math.min(ir.bottom, lr.bottom) - Math.max(ir.top, lr.top)
        if (overlap > bestArea) { bestArea = overlap; bestIdx = parseInt((item as HTMLElement).dataset.roundIdx || '0') }
      })
      if (bestIdx !== currentRoundIdx) { currentRoundIdx = bestIdx; refreshCurrentHighlight() }
      ticking = false
    })
  })
}

function ensurePanelSkeleton(roundsCount: number, messagesCount: number): [HTMLElement, HTMLElement] {
  let panel = shadowRoot!.querySelector('.panel') as HTMLElement | null
  if (panel) {
    const titleEl = panel.querySelector('.title')
    if (titleEl) titleEl.textContent = roundsCount + ' rounds · ' + messagesCount + ' messages'
    const sumBtn = panel.querySelector('.summary-btn') as HTMLButtonElement | null
  ...
  orderBtn.title = reverseOrder ? 'Newest first (click to reverse)' : 'Oldest first (click to reverse)'
  orderBtn.onclick = () => { reverseOrder = !reverseOrder; refreshUI() }
  ...
  closeBtn.innerHTML = ICON_X_SVG
  closeBtn.onclick = () => { isOpen = false; refreshUI() }
  ...
  searchInput.oninput = () => { searchQuery = searchInput.value.toLowerCase().trim(); refreshUI() }
...
function refreshUI() {
  ...
  const rounds = reverseOrder ? groupIntoRounds(newMessages).reverse() : groupIntoRounds(newMessages)
  const newIds = rounds.map(r => r.id)

  // Fast-path: nothing structurally changed AND same UI mode AND no search active
  if (!searchQuery && !prevSearchActive &&
    ...
    if (titleEl) titleEl.textContent = rounds.length + ' rounds · ' + newMessages.length + ' messages'
    prevRoundIds = newIds
    return
  }
  prevRoundIds = newIds
  ...
  if (!highlightInitialized && !searchQuery) { currentRoundIdx = 0; highlightInitialized = true; setupScrollHighlight() }
...
function setupObserver() {
  ...
  observer = new MutationObserver(() => {
    ...
    if ((window as unknown as { __aiSidebarDebounce?: number }).__aiSidebarDebounce) {
      clearTimeout((window as unknown as { __aiSidebarDebounce?: number }).__aiSidebarDebounce!)
    }
    ;(window as unknown as { __aiSidebarDebounce?: number }).__aiSidebarDebounce = window.setTimeout(() => { if (!isDragging) { refreshUI(); setupHistoryTitleEditing() } }, 250)
  })
  if (document.body) {
...
function setupHistoryTitleEditing() {
  ...
  items.forEach((item) => {
    ...
    itemEl.addEventListener('dblclick', (e) => {
      e.preventDefault()
      e.stopPropagation()
      const target = (e.target as HTMLElement).closest('span, div, p') || itemEl
      const oldText = (target.textContent || '').trim()
      const input = document.createElement('input')
      input.type = 'text'
      input.value = oldText
      input.style.cssText = 'width: 100%; min-width: 0; font: inherit; background: white; border: 1px solid #3b82f6; padding: 2px 4px; border-radius: 3px; color: black;'
      target.textContent = ''
      ...
      const save = () => {
        ...
        try { chrome.storage.local.set({ [storageKey]: newText }) } catch {}
...
function applyCustomTitle(anchor: Element, customTitle: string) {
  ...
  candidates.forEach((el) => {
    ...
    if (t.length > bestLen) { best = el as HTMLElement; bestLen = t.length }
  })
...
function setupPeriodicPush() {
  if ((window as unknown as { __aiSidebarInterval?: number }).__aiSidebarInterval) return
  ;(window as unknown as { __aiSidebarInterval?: number }).__aiSidebarInterval = window.setInterval(() => { if (!isDragging) refreshUI() }, 3000)
}
